...
// NetDevFeaturesSet applies named feature changes to ifIndex. Features absent
// from config are left unchanged.
...
func (h *Handle) NetDevFeaturesSet(ifIndex int, config map[string]bool) error {
	...
	_, err = h.ethtoolRequest(nl.ETHTOOL_MSG_FEATURES_SET, unix.NLM_F_ACK, attrs)
	return err
}
