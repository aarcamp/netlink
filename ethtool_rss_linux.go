...
func parseNetDevRSS(attrs []syscall.NetlinkRouteAttr) (*NetDevRSS, error) {
	rss := &NetDevRSS{}
...
func (h *Handle) NetDevRSSGet(ifIndex int, context uint32) (*NetDevRSS, error) {
	...
	rss, err := parseNetDevRSS(msgs[0])
	if err != nil {
		return nil, err
	}
	if rss.Context != context {
		return nil, fmt.Errorf("netlink: ethtool RSS response context is %d, want %d", rss.Context, context)
	}
	return rss, nil
}
...
func newNetDevRSSSetAttrs(ifIndex int, context uint32, config NetDevRSSConfig) ([]*nl.RtAttr, error) {
	...
	if config.HashFunction != nil {
		if *config.HashFunction == 0 || *config.HashFunction > 0xff {
			return nil, fmt.Errorf("netlink: invalid RSS hash function %#x", *config.HashFunction)
		}
		attrs = append(attrs, nl.NewRtAttr(nl.ETHTOOL_A_RSS_HFUNC, nl.Uint32Attr(uint32(*config.HashFunction))))
	}
